function getIncSize(fs, totals = []) {
    ...
    for (const key of keys) {
        if (Number.isInteger(fs[key])) {
            ...
            total += getIncSize(fs[key], totals)[0];
        }
    }
...
function parse(input) {
    ...
    return getIncSize(fileSystem);
}

const getButchForDeletion = (input) => {
    const totals = parse(input);
    return totals.filter(x => x <= 100000).reduce((a, b) => a + b);
}

const getSmallestDirFromButch = (input) => {
    const totals = parse(input);
