...
const calcScore = (lines, i, j) => {
    let score1 = 0;
    for (let xi = i + 1; xi < lines.length; xi++) {
        score1++;

        if (lines[xi][j].height >= lines[i][j].height) {
            break;
        }
    }
    ...
    for (let xj = j + 1; xj < lines[0].length; xj++) {
        score3++;
        if (lines[i][xj].height >= lines[i][j].height) {
            break;
        }
    }
...
const getCountOfVisibleTrees = (input) => {
    const map = input
        .split('\n')
...
const getHighestScenicScore = (input) => {
    const lines = input
        .split('\n')
